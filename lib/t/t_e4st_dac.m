...
mpopt = mpoption(mpopt, 'opf.violation', 5e-8, 'mips.comptol', 1e-8, 'mips.step_control', 0);
...
if have_fcn('glpk') || have_fcn('gurobi') || have_fcn('cplex') || ...
        ...
        %% load the case
    for testnum = 1:2    
        casename = sprintf('t_case_e4st_dac%d', testnum);
        solution = sprintf('t_e4st_dac_soln%d', testnum);
...
%         %-------save solution--------------------
%         fname = sprintf('t_e4st_dac_soln%d', testnum);
